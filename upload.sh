...
#修改spec文件tag
#获取版本号并显示
VersionString=`grep -E "s.version.*=" GHFirebase.podspec`
...
LineNumber=`grep -nE 's.version.*=' GHFirebase.podspec | cut -d : -f1`
sed -i "" "${LineNumber}s/${VersionNumber}/${NewVersionNumber}/g" GHFirebase.podspec
echo -e "\n${Default}================================================"
